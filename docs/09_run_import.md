...
Загрузчик читает фактическую строку заголовков `ЖурналОпераций` и раскладывает поля по ней, поэтому скрытые технические колонки могут оставаться в таблице. В видимой части журнала используются поступление, списание, денежный счёт, статьи ДДС/ОПиУ, проект, контрагент, назначение, Дт, Кт и балансовая статья. Правила импорта и учёта в Google Sheets не хранятся: источником истины служит проектный JSON в репозитории.
